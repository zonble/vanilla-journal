...
vj.util = {
  exitconfirm: function () {
    var agree = window.confirm(
      "使用此功能將會離開目前的頁面。如果您不儲存資料，您將無法保存您剛才所輸入的各項資料。您確定要離開嗎？"
    );
...
vj.block = {
  ...
  hide: function (url) {
    if ($("vjblock")) {
      new Effect.Fade("vjblock", { duration: 0.2 });
    }
    if ($("overlay")) {
  ...
  show: function (url) {
    ...
    $("vjblock").innerHTML =
      '<p><a id="closewin" href="#" accesskey="w" title="快速鍵：PC 上為 Alt + W，麥金塔系統為 Ctrl + W">關閉視窗</a></p><iframe id="vjiframe"></iframe>';
...
vj.post.images = {
  init: function () {
    ...
    Sortable.create("image_block", {
      dropOnEmpty: true,
      tag: "div",
      containment: ["image_block"],
      constraint: false,
      scroll: true,
      overlap: "horizontal",
      onChange: function (el) {
        vj.post.images.sort(el);
      },
    });
    vj.post.images.sort();
    if ($("upload_img")) {
    ...
    if ($("image_form_reset")) {
      $("image_form_reset").onclick = function () {
        ...
        new Ajax.Request(url, {
          method: "get",
          onComplete: vj.post.images.update,
        });
        return false;
      };
    }
  ...
  ordersubmit: function () {
    ...
    new Ajax.Request(url, {
      method: "post",
      parameters: pars,
      onComplete: vj.post.images.orderover,
    });
    return false;
  },
};
  ...
  init: function () {
    new Ajax.Autocompleter(
      ...
      "post-author.php",
      {
        minChars: 1,
        paramName: "author",
        afterUpdateElement: vj.post.authordesc.update,
      }
    );
  },
  ...
  update: function () {
    if ($("author_desc") && $("author")) {
      ...
      new Ajax.Request(url, {
        method: "get",
        parameters: pars,
        onComplete: vj.post.authordesc.change,
      });
    }
  },
    ...
    if ($("author_desc") && $("author")) {
      if ($("author_desc").value != r.responseText) {
        ...
        if ($("author_desc").value == "") {
          ...
          message =
            "系統中已經有這位作者的簡介了，而且與您現在輸入的作者介紹不同。您想要使用之前的介紹文字嗎？";
    ...
    $("cat-all").onchange = function () {
      if (cats) {
        for (i = 0; i < cats.length; i++) {
          cats[i].selectedIndex = this.selectedIndex;
        }
      }
    };
    ...
    $("volume").onchange = function () {
      if (vols) {
        for (i = 0; i < vols.length; i++) {
          vols[i].selectedIndex = this.selectedIndex;
        }
      }
    };
  },
...
vj.init = function () {
  ...
  if (
    !(
      BrowserDetect.browser in
      { Explorer: 1, Firefox: 1, Camino: 1, Mozilla: 1, Safari: 1 }
    )
  ) {
    vj.notsupport = 1;
  }
  ...
  if ($("voltool")) {
    ...
    if (!vj.notsupport) {
      ...
      if ($("vol_extra")) {
        Sortable.create("vol_extra", {
          dropOnEmpty: true,
          tag: "div",
          containment: ["vol_extra"],
          constraint: false,
          overlap: "horizontal",
          onChange: function (el) {
            vj.volume.sortcat(el);
          },
        });
        vj.volume.sortcat();
      }
  ...
  if ($("infotool")) {
    vj.info.show_area("info_basic");
  }

  if ($("filetool")) {
    vj.file.show_area("file_theme");
  }
  ...
  vj.block.init();
  if ($("msg")) {
    new Effect.Fade("msg", { duration: 3.0 });
  }
  if ($("quicklink")) {
  ...
  if ($("post_table")) {
    ...
    for (i = 1; i <= tables.length; i++) {
      ...
      Sortable.create(tbodyname, {
        dropOnEmpty: true,
        tag: "tr",
        containment: [tbodyname],
        constraint: false,
        handle: "handle",
        onChange: function (el) {
          vj.volume.sortpost(el);
        },
      });
    }
  }
