...
var Enumerable = {
  ...
  inGroupsOf: function (number, fillWith) {
    ...
    if (results.length > 0)
      (number - results.last().length).times(function () {
...
Ajax.Request.prototype = Object.extend(new Ajax.Base(), {
  ...
  request: function (url) {
    ...
    if (this.options.method == "get" && params.any())
      this.url +=
        (this.url.indexOf("?") >= 0 ? "&" : "?") + params.toQueryString();
    ...
    try {
      ...
      var body =
        this.options.method == "post"
          ? this.options.postBody || params.toQueryString()
          : null;
...
Element.extend = function (element) {
  ...
  if (!element._extended && element.tagName && element != window) {
    ...
    var methods = Object.clone(Element.Methods.Simulated),
      cache = Element.extend.cache;
    for (var property in methods) {
      var value = methods[property];
...
Object.extend(Selector, {
  matchElements: function (elements, expression) {
    ...
    return elements
      .select(selector.match.bind(selector))
      .collect(Element.extend);
  },
...
var Form = {
  ...
  serializeElements: function (elements) {
    return elements
      .inject([], function (queryComponents, element) {
