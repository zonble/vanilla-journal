...
>
> 這是我在 2006 年於學校工作時（2004-2007 年，政大傳播學院電腦室助教），為了系所的電子報所開發的輕量級管理系統。本專案以開源方式釋出，隨著離職後已停止維護。它完美保留了 2000 年代中期（Web 1.0 晚期至 Web 2.0 早期）的 PHP 網頁開發面貌與技術堆疊。
...
## 當時的工程巧思 (Engineering Excellence of 2007)

儘管以現代標準來看它有些陳舊，但在 2007 年的時空背景下，這份專案展現了相當優異的工程素養：

- **高度物件導向化**：在許多 PHP 開發者還在寫純程序化 (Procedural) 代碼的時代，本專案就已經廣泛使用類別 (Class) 來封裝功能，如文章、期數、附件等都有對應的物件處理邏輯。
- **靈活的佈景主題架構**：內建支援多種佈景主題切換，這在當時是非常進階且具備擴充性的設計思維。
- **對 Web 2.0 趨勢的掌握**：整合了當時最領先的 `Prototype.js` 與 `Script.aculo.us` 框架，提供了流暢的 AJAX 互動與視覺特效，這在當時的大專院校網站中是非常罕見的高水準表現。
- **嚴謹的檔案組織**：檔案結構清晰、命名具備一致性，且在 `config.php.sample` 等關鍵檔案中留下了詳盡的中文註解，充分展現了對後續維護者的友善。
...
- **SSH & Vim 實戰開發**：當時的「線上即時熱修復 (Hotfix)」就是直接 SSH 進入主機，用 `vim` 修改 PHP 檔案。沒有 PR Review 流程，也沒有暫存環境，改完存檔（`:wq`）就是正式上線。
- **完全不具備響應式設計 (No RWD)**：在第一代 iPhone 剛問世、大眾還在用 Nokia N 系列或傳統手機的年代，Web 開發完全不考慮手機螢幕。沒有 Media Queries，也沒有彈性佈局。網頁就是為了 1024x768 或 800x600 的電腦螢幕而生。
- **CI/CD：哪有這種東西？** 在那個年代，唯一的 CI/CD 流程就是開發者的大腦與手動 FTP。沒有自動化測試、沒有 GitHub Actions、沒有 Jenkins。所謂的「部署」通常是透過 FTP 軟體將修改過的檔案直接覆蓋到伺服器上，或者是直接在伺服器上透過線上編輯器修改程式碼。
